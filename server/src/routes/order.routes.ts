...
router.post('/', zodBody(createOrderSchema), order.createOrder);
...
router.patch('/:id/status', requirePermission('orders', 'update'), zodBody(updateStatusSchema), logActivity('status', 'orders'), order.updateStatus);
